...
if ($type == 'delete-all-events'){
    ...
    $stmt = $dbh->prepare("DELETE FROM events WHERE parent_id='$parent_id' AND CONCAT_WS(DATE(start),' ',start)>= CONCAT_WS(DATE(start),' ','$start_date')");
    ...
    $dbh->beginTransaction();
        ...
        foreach ($resources as $row) {
            $stmt = $dbh->prepare("UPDATE events SET description=:description, category_id=1 WHERE DATE(start)='$start_date' AND
                                 ...
                                 (TIME(start)='$start_time' AND TIME(start)<'$end_time') OR
                                 (TIME(end)='$end_time' AND TIME(end)>'$start_time')) AND category_id=6");
            $firephp->log("petla");
...
//delete all events with repeats
if ($type == 'delete-all-events-repeats'){
    $parent_id = $_POST['parent_id'];
    $event_id = $_POST['event_id'];
    $start_date = $_POST['start_date'];
    $start_time = $_POST['start_time'];
...
if ($type == 'delete-child-event'){
    ...
    $stmt = $dbh->prepare("DELETE FROM events WHERE parent_id='$parent_id' AND start='$start'");
    $stmt->execute();
...
if ($type == 'updateDescription'){
    ...
    try{
        $dbh->beginTransaction();
        $dbh->query('SET NAMES utf8');
        $stmt = $dbh->prepare("UPDATE events SET description=:description WHERE parent_id=:parent_id");
        $stmt->bindParam(':description', $description);
